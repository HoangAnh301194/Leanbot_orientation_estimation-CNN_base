...
def render_bbox_overlay(
    ...
    if label_y < 20:
        label_y = y1 + 20
    label = f"Obj{object_index + 1} {short_class_label(best_class)}: {best_conf:.3f}"
    check_confidence.draw_label(image, label, x1, label_y, color)


def run_low_level_inference(model, frame: np.ndarray, args: argparse.Namespace) -> tuple[np.ndarray, list[dict]]:
    ...
    if detections is None or len(detections) == 0:
        return bbox_image, detection_data
...
def main() -> None:
    ...
    for image_path in images:
        ...
        bbox_image, detections = run_low_level_inference(model, frame, args)

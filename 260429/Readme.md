...
```cpp
// opencv2/objdetect.hpp — class SimilarRects
class SimilarRects {
public:
...
- **Giải thích logic so sánh:**
    - `delta = eps * (min(w1, w2) + min(h1, h2)) / 2` — ngưỡng sai khác được tính tương đối theo kích thước nhỏ nhất của 2 rectangle.

...
**Đoạn code thực hiện Lượng tử hóa (Quantization FP16):**
```python
from ultralytics import YOLO

# Load model PyTorch gốc
model_pt = YOLO('best_24Class_Soft_Angular_BCE.pt')

# 1. Export sang định dạng ONNX (Quantize FP16) bằng tham số half=True
onnx_fp16_path = model_pt.export(format="onnx", imgsz=640, half=True)

# 2. Export sang định dạng OpenVINO (Quantize FP16) bằng tham số half=True

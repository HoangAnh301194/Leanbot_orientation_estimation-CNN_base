...
def plot_online_poly_log(csv_file: str, n_sample_segments: int = 3, poly_degree: int = 2):
    ...
    # Subplot 1: So sánh Raw Angle vs Smooth Angle (Unwrapped)
    ax_ang = axes[0]
    has_angle = 'angle' in df.columns or 'group1_angle' in df.columns
    if has_angle:
        ang_col = 'group1_angle' if 'group1_angle' in df.columns else 'angle'
        unwrapped_raw_angle = np.degrees(np.unwrap(np.radians(df[ang_col])))
        ax_ang.plot(df['frame_id'], unwrapped_raw_angle, 'b.-', linewidth=1.0, markersize=2, alpha=0.6, label='Raw Angle (Inference)')

    if has_smooth:
        dx = df['smooth_x'].diff()
        dy = df['smooth_y'].diff()
        # -dy để chuyển từ hệ tọa độ ảnh OpenCV (Y hướng xuống) sang hệ Đề-các (Y hướng lên)
        smooth_ang_deg = np.degrees(np.arctan2(-dy, dx))
        if has_angle:
            smooth_ang_deg.iloc[0] = df[ang_col].iloc[0]
        else:
            smooth_ang_deg.iloc[0] = 0.0
        smooth_ang_deg = smooth_ang_deg.bfill().ffill()
        unwrapped_smooth_angle = np.degrees(np.unwrap(np.radians(smooth_ang_deg)))
        ax_ang.plot(df['frame_id'], unwrapped_smooth_angle, color='#ff7f00', linewidth=1.6, label='Smooth Angle (Trajectory Vector)')

    ax_ang.set_ylabel('Degrees')
    ax_ang.grid(True, linestyle='--', alpha=0.7)
    ax_ang.legend(loc='upper right')
    ax_ang.set_title("Heading Angle Comparison: Raw (Inference) vs Smooth (Trajectory)")

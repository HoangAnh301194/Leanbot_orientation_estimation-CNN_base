...
class PositionPIDController:
    ...
    def _crop_speed(self, speed: float) -> int:
        return int(round(max(-self.max_velocity, min(speed, self.max_velocity))))
    ...
    def compute(
        ...
        if self.phase == self.PHASE_ALIGNING:
            ...
            speed_left = self._crop_speed(v_lr + v_diff)
            speed_right = self._crop_speed(v_lr - v_diff)
            debug = self._debug(
        ...
        # Calculate and crop individual wheel speeds

...
def calculate_ellipse_phase_angles(x, y, ellipse_info, cartesian=False):
    center_x, center_y = ellipse_info['center']
    reference_angle_image = float(ellipse_info['angle'])
    ...
    delta_y_cartesian = center_y - np.asarray(y, dtype=float)

    radial_angles = np.degrees(np.arctan2(delta_y_cartesian, delta_x))
    wrapped_angles = np.mod(radial_angles - reference_angle_cartesian, 360.0)
    continuous_angles = np.degrees(np.unwrap(np.radians(wrapped_angles)))

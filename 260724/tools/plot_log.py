...
def plot_log(csv_file: str):
    ...
    has_iou_cols = 'iou_prev_bbox' in df_plot.columns
    n_axes = 2
    if has_mag_cols:
        n_axes += 1
    if has_iou_cols:
    ...
    # ---------------------------------------------------------
    # 4. Đồ thị IoU giữa BBox frame trước và frame hiện tại
    # ---------------------------------------------------------
    if has_iou_cols:
        ax_iou = axes[ax_idx]
        ax_iou.plot(df_plot['frame_id'], df_plot['iou_prev_bbox'], 'm.-', label='IoU (Current vs Prev BBox)', linewidth=1.5, markersize=4)
        ax_iou.axhline(y=0.5, color='orange', linestyle='--', alpha=0.7, label='IoU Threshold 0.5')
        ax_iou.set_ylabel('IoU')
        ax_iou.set_ylim(-0.05, 1.05)
        ax_iou.grid(True, linestyle='--', alpha=0.7)
        ax_iou.legend(loc='lower right')
        ax_iou.set_title("BBox IoU Consecutive Frames")
        ax_idx += 1

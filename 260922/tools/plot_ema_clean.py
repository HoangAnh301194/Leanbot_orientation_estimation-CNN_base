...
def run_clean_ema_experiments(csv_file: str, seg_length: int = 30, num_segments: int = 3, seed: int = 42):
    ...
    img_w = 1280
    img_h = 720
    if 'frame_width' in valid_df.columns and valid_df['frame_width'].iloc[0] > 160:
        img_w = int(valid_df['frame_width'].iloc[0])
    if 'frame_height' in valid_df.columns and valid_df['frame_height'].iloc[0] > 160:
        img_h = int(valid_df['frame_height'].iloc[0])
    ...
    ax_traj.set_title(f'2D Trajectory (Full Frame {img_w}x{img_h}) - Raw vs EMA (alpha = 0.5, 0.7, 0.9)\nFile: {csv_path.name}', fontsize=14, fontweight='bold')
    ...
    ax_traj4.set_xlim(0, img_w)
    ax_traj4.set_ylim(img_h, 0)
    ax_traj4.set_title(f'2D Trajectory (Full Frame {img_w}x{img_h}) - Raw vs EMA Trajectory (alpha = 0.5, 0.7, 0.9)\nFile: {csv_path.name}', fontsize=14, fontweight='bold')

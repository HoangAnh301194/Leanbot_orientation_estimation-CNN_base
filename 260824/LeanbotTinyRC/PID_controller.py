...
# --- HANG SO HE THONG ---
MAX_VELOCITY: int = 2000           # Van toc toi da truyen cho dong co Leanbot
MIN_START_VELOCITY: int = 5     # Van toc toi thieu de thang ma sat tinh (Deadband compensation)
DEFAULT_DEADZONE_DEG: float = 1.0  # Vung chet sai so goc (+/- 1.0 do)
DEFAULT_DIST_TOLERANCE_PX: float = 10.0  # Dung sai khoang cach pixel de coi la den dich
DEFAULT_ANGLE_INTEGRAL_THRESHOLD_DEG: float = 15.0  # Chi kich hoat Ki khi sai so goc <= 15 do
DEFAULT_DIST_INTEGRAL_THRESHOLD_PX: float = 50.0  # Chi kich hoat Ki khi sai so khoang cach <= 50 px
...
class PIDController:
    ...
    def __init__(
        ...
        max_i: float = 300.0,
        integral_activation_threshold_deg: float = DEFAULT_ANGLE_INTEGRAL_THRESHOLD_DEG,
        ...
        self.max_i = max_i
        self.integral_activation_threshold = integral_activation_threshold_deg
        ...
        self.aligned_start_time: Optional[float] = None
    ...
    def compute(
        ...
        if abs(error) <= self.deadzone_deg:
            if self.aligned_start_time is None:
                self.aligned_start_time = now
            
            is_aligned = (now - self.aligned_start_time) >= (self.hold_time_ms / 1000.0)
        ...
        # 4. Integral co dieu kien & Anti-windup
        # Chi cong don khi sai so da giam vao vung can bu tinh.
        if abs(error) <= self.integral_activation_threshold:
...
class PositionPIDController:
    def __init__(
        ...
        angle_integral_activation_threshold_deg: float = DEFAULT_ANGLE_INTEGRAL_THRESHOLD_DEG,
        dist_integral_activation_threshold_px: float = DEFAULT_DIST_INTEGRAL_THRESHOLD_PX,
        hold_time_ms: float = 100.0,
    ):
        self.angle_pid = PIDController(
            ...
            integral_activation_threshold_deg=angle_integral_activation_threshold_deg,
        ...
        self.dist_integral_activation_threshold = dist_integral_activation_threshold_px
        self.hold_time_ms = hold_time_ms
        
        # Bien trang thai PID khoang cach
        self.dist_integral: float = 0.0
        self.prev_dist_error: Optional[float] = None
        self.prev_time: Optional[float] = None
        self.pos_aligned_start_time: Optional[float] = None
    ...
    def reset(self):
        ...
        self.prev_time = None
        self.pos_aligned_start_time = None
    ...
    def compute(
        ...
        if dist <= self.dist_tolerance:
            ...
            if final_heading_deg is not None:
                ...
                if self.pos_aligned_start_time is None:
                    self.pos_aligned_start_time = now
                
                is_completed = (now - self.pos_aligned_start_time) >= (self.hold_time_ms / 1000.0)
                
                if is_completed:
                    self.reset()
                
                debug_info = {
                    "state": "COMPLETED" if is_completed else "AT_SETPOINT",
                    "distance": dist,
                    "angle_error": 0.0,
                    "v_linear": 0,
                    "u_angular": 0,
        ...
        # PID khoang cach voi tich phan co dieu kien & Anti-windup
        P_dist = self.Kp_dist * dist
        if dist <= self.dist_integral_activation_threshold:

# Cập nhật báo cáo ngày 07/05/2026

## A. Công việc đã làm
- Báo cáo lại tập Datasets
- Giải thích các thông tin liên quan đến Training Model và link tham khảo :
  - Image size model input
  - metrics
- Cập nhật link thông tin Data Augmentation có sẵn trong YOLO training 


---

### 1. Bảng thông tin Dataset cho Training

#### 1.1. Thông tin các Session thu thập


| Session | Class | Số ảnh Background | Số ảnh Raw | Số nhãn | Số ảnh có số nhãn tương ứng|
| :--- | :---: | :---: | :---: | :---: | :---: |
| `session_20260504_090104` | `Leanbot_back` | 1 | 7 | 9 | 7 |
| `session_20260504_090516` | `Leanbot_front` | 1 | 7 | 9 | 7 |
| `session_20260504_102751` | `Leanbot_front` | 1 | 14 | 9 | 14 |
| `session_20260504_103507` | `Leanbot_back` | 1 | 14 | 9 | 14 |
| **Tổng cộng** | | **4** | **42** | **378** | **42** |

#### 1.2. Thống kê theo class :

| Class | Class ID | Số ảnh | Số nhãn |
| :--- | :---: | :---: | :---: |
| `Leanbot_front` | 0 | 21 | 189 |
| `Leanbot_back` | 1 | 21 | 189 |
| **Tổng** | | **42** | **378** |
...
##### 1.4.1. Bảng so sánh thông tin Training giữa 2 mô hình

| Thông số | 1 Class (`Leanbot`) | 2 Class (`Leanbot_front`, `Leanbot_back`) |
| :--- | :---: | :---: |
| Model nền tảng | `yolov8n.pt` (YOLOv8 Nano - Pre-trained COCO) | `yolov8n.pt` (YOLOv8 Nano - Pre-trained COCO) |
| Số class | 1 | 2 |
| Epochs | 100 | 100 |
| Batch size | 16 | 16 |
| Image size | 640 × 640 | 640 × 640 |
| Optimizer | AdamW (lr=0.002) | AdamW (lr=0.002) |
| Thời gian Training | ~2.5 phút (0.041 hours) | ~4 phút |
| Môi trường Training | Google Colab (GPU Tesla T4) | Google Colab (GPU Tesla T4) |
| Link Notebook | [finetuning_yolo_Leanbot.ipynb](https://git.pythaverse.space/thomha/Nguyen_Huu_Hoang_Anh/blob/master/260507/tools/finetuning_yolo_Leanbot.ipynb) | [finetuning_yolo_Leanbot.ipynb](https://git.pythaverse.space/thomha/Nguyen_Huu_Hoang_Anh/blob/master/260505/tools/finetuning_yolo_Leanbot.ipynb) |

##### 1.4.2. Kết quả cuối cùng (Best Model) trên tập Validation

| Metric | 1 Class (`Leanbot`) | 2 Class (`Front` / `Back`) |
| :--- | :---: | :---: |
| **Precision (P)** | 0.997 | 1.00 |
| **Recall (R)** | 1.00 | 1.00 |
| **mAP@0.5** | 0.995 | 0.995 |
| **mAP@0.5:0.95** | 0.850 | ~0.900 |
| **F1-Score** | ~1.00 (tại conf=0.725) | ~1.00 (tại conf=0.689) |

##### 1.4.3. Kết quả nhận diện trên tập Test

| Mô hình | Mô tả kết quả |
| :--- | :--- |
| **1 Class** | Phát hiện đúng tất cả Leanbot trên tập test với confidence từ **0.85 – 0.94**. Bounding box chính xác, không có False Positive. |
| **2 Class (Front)** | Phát hiện đúng tất cả `Leanbot_front` với confidence từ **0.88 – 0.98**. Không nhầm lẫn với mặt sau. |
| **2 Class (Back)** | Phát hiện đúng tất cả `Leanbot_back` với confidence từ **0.86 – 0.97**. Không nhầm lẫn với mặt trước. |

##### 1.4.4. Image size (imgsz) là gì?

`imgsz` (Image size) là tham số xác định **kích thước đầu vào** mà mô hình YOLO nhận để xử lý trong quá trình training, validation và inference.

- **Cách hoạt động**: Khi ảnh gốc có kích thước khác với `imgsz`, YOLO sẽ tự động **resize** ảnh về đúng kích thước đích (ví dụ 640×640) bằng kỹ thuật **Letterboxing** — tức là giữ nguyên tỉ lệ khung hình (aspect ratio) và thêm viền padding (thường là màu đen, mức xám = 0) để đạt kích thước vuông.
- **Yêu cầu**: `imgsz` phải chia hết cho **stride** của model (thường là 32). Ví dụ: 640, 1024, 1280 đều hợp lệ.
- **Trade-off**:
  - `imgsz` **lớn hơn** → phát hiện được vật thể nhỏ tốt hơn, nhưng tốn nhiều VRAM và chậm hơn.
  - `imgsz` **nhỏ hơn** → nhanh hơn, nhưng có thể bỏ sót vật thể nhỏ.
- **Thực tế trong project**: Sử dụng `imgsz=640` (mặc định), phù hợp với bộ dữ liệu có nhiều Leanbot nhỏ trong ảnh.
...
>
> **Link tham khảo giải thích chi tiết các Metric**: [Ultralytics – YOLO Performance Metrics](https://docs.ultralytics.com/guides/yolo-performance-metrics/)
...
> - [Ultralytics – Training Settings (Augmentation Parameters)](https://docs.ultralytics.com/modes/train/#augmentation-settings-and-hyperparameters)
...
- Thu thập ảnh Right/Left với các góc 0, +-15, +-30 , training model và đánh giá.

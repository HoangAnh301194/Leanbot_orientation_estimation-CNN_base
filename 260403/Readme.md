...
- [A. Công việc đã làm](#a-công-việc-đã-làm)
  ...
  - [2. Kiểm thử Camera](#2-kiểm-thử-camera)
  - [3. Calibration Sa bàn cho Camera bằng OpenCV](#3-calibration-sa-bàn-cho-camera-bằng-opencv)
    - [3.1. Cơ sở lý thuyết](#31-cơ-sở-lý-thuyết)
      - [3.1.1. Hình học tạo ảnh và phép biến đổi phối cảnh](#311-hình-học-tạo-ảnh-và-phép-biến-đổi-phối-cảnh-geometry-of-image-formation)
      ...
      - [3.1.2. Camera Calibration](#312-camera-calibration-hiệu-chỉnh-camera)
        - [a. Méo ống kính](#a-méo-ống-kính-lens-distortion)
        - [b. Mục tiêu của Camera Calibration](#b-mục-tiêu-của-camera-calibration)
        - [c. Phương pháp Calibration bằng bàn cờ](#c-phương-pháp-calibration-bằng-bàn-cờ-checkerboard-pattern)
        - [d. Bản chất hàm findchessboardcorner](#d-bản-chất-hàm-findchessboardcorner)
        - [e. Ứng dụng kết quả Calibration](#e-ứng-dụng-kết-quả-calibration)
  - [4. Triển khai Calibration Cam và Hiệu chỉnh sa bàn](#4-triển-khai-calibration-cam-và-hiệu-chỉnh-sa-bàn)
- [B. Khó khăn](#b-khó-khăn)
- [C. Công việc tiếp theo](#c-công-việc-tiếp-theo)

### 1. Tìm các giá gắn cam, tripod trên thị trường
- Em đã tìm và đưa ra một số mẫu trong link sheet sau dựa theo một số đề mục như : Giá cả, nơi bán, số lượt mua, chiều cao: 
[https://docs.google.com/spreadsheets/d/14kZN-g951N4ITxF6UKQRfKNe8BYNenRhpWN_Cq4076k/edit?gid=0#gid=0](https://docs.google.com/spreadsheets/d/14kZN-g951N4ITxF6UKQRfKNe8BYNenRhpWN_Cq4076k/edit?gid=0#gid=0)
...
### 3. Calibration Sa bàn cho Camera bằng OpenCV
#### 3.1. Cơ sở lý thuyết 
...
##### 3.1.1. Hình học tạo ảnh và phép biến đổi phối cảnh (Geometry of Image Formation)
...
##### 3.1.2. Camera Calibration (Hiệu chỉnh Camera)
...
###### d. Bản chất hàm findchessboardcorner

**Bản chất hàm `cv2.findChessboardCorners()` trong OpenCV:**

Đây là một hàm dựng sẵn thư viện OpenCV nằm trong module `calib3d`.

Về mặt thuật toán, nó hoạt động theo logic toàn cục (All-or-Nothing) qua 4 tiến trình:
- **Phân ngưỡng ảnh (Binarization):** Chuyển ảnh màu gốc sang ảnh xám (Grayscale), sau đó áp dụng dải ngưỡng cục bộ (Adaptive Threshold) biến mọi thứ thành Đen và Trắng tuyệt đối để tối đa hóa độ tương phản.
- **Tìm tứ giác Đen (Quad Detection):** Thuật toán dò tất cả các đa giác có 4 cạnh màu Đen trên khung hình.
- **Định vị điểm giao (Corner Approximation):** Nối các đỉnh vát chéo lân cận của các ô đen để xác định một điểm tâm ở giữa, gọi là Tọa độ điểm góc chéo (Inner Corners).
- **Khớp Ma trận lưới (Pattern Matching):** Sắp xếp và đếm số điểm giao đó để ráp nối vào một mạng lưới (Grid) theo cấu hình truyền vào (ở trong code Calibrate em đã chọn là 4x4).

###### e. Ứng dụng kết quả Calibration
...
### 4. Triển khai Calibration Cam và Hiệu chỉnh sa bàn.
- Phương pháp triển khai:
  ...
  **b. Tính toán ma trận nội tham số, ngoại tham số và hệ số méo**
  - **Công cụ:** Chạy script `calibrate_camera.py`.
  - **Xử lý điểm ảnh:** Tải toàn bộ Data ảnh từ bước trên, sử dụng `cv2.cornerSubPix` để tìm tọa độ của các góc nhọn trên bàn cờ.
  - **Giải hệ phương trình:** Gọi hàm lõi `cv2.calibrateCamera` để trích toàn bộ đặc trưng vật lý của Camera:
    - *Camera Matrix (Nội tham số):* Tìm được tiêu cự $f_x, f_y$ và tâm quang học $c_x, c_y$.
    - *Distortion Coefficients (Hệ số méo):* Tìm các hệ số méo Radial và Tangential.
    - *Ngoại tham số phụ:* Trả về Rotation và Translation Vectors đóng vai trò tọa độ Camera so với mặt phẳng giấy Caro tại đúng thời điểm bấm máy.
  - **Đánh giá và trích xuất:** Sai số bù hình chung (`Re-projection Error`) được in ra màn hình để nghiệm thu chất lượng (mức độ tốt < 0.5 pixel). Kết quả được đóng block lưu vào tệp `calibration_result.npz`; tự động khởi tạo thư mục `comparison_results` lưu lại ảnh Trước - Sau khi khử méo để trực quan kiểm chứng.
...
## B. Khó khăn
- Em đã triển khai được Calibration, tuy nhiên vì chưa có giá gắn Cam nên chưa kiểm nghiệm được sai số.

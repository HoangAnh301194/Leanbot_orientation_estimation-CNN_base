...
DEFAULT_HEADING_TOLERANCE3_DEG = 5.0  # Phase 3 narrower heading tolerance
DEFAULT_SETTLE_TIME_MS = 500.0  # Stability duration in ms before complete
DEFAULT_FWD_BWD_TIME_S = 3.0  # Duration in seconds for forward & backward test
DEFAULT_FWD_BWD_SPEED = 2000  # Wheel speed for forward & backward test
...
class PositionPIDController:
    ...
    PHASE_FWD_BWD = 'PHASE_4_FWD_BWD'
    PHASE_FORWARD = 'PHASE_4_FWD_BWD'   # Alias for backward compatibility
    PHASE_BACKWARD = 'PHASE_4_BACKWARD' # Legacy alias
    ...
    def __init__(
        ...
        self._rfb_start_time: Optional[float] = None
        self._rfb_triggered: bool = False
    ...
    def reset(self) -> None:
        ...
        self._rfb_start_time = None
        self._rfb_triggered = False
    ...
    def compute(
        ...
        if self.phase == self.PHASE_FINAL_ALIGNING:
            ...
            if abs(final_angle_error) <= self.heading_tolerance3:
                ...
                if elapsed_ms >= self.settle_time_ms:
                    # If Phase 4 enabled (fwd_bwd_time_s > 0), transition to Phase 4 Autonomous Fwd/Bwd (rfb)
                    if self.fwd_bwd_time_s > 0.0:
                        self.phase = self.PHASE_FWD_BWD
                        self._rfb_start_time = current_time
                        self._rfb_triggered = False
                    else:
                        self.phase = self.PHASE_COMPLETED
                        debug = self._debug(
            ...
            if self.phase == self.PHASE_FINAL_ALIGNING:
                # Phase 3 uses dedicated Phase 3 PID gains for spinning in place
                ...
                v_lr = 0.0
                v_diff = (self.Kp_angle3 * final_angle_error
                          + self.Ki_angle3 * self._integral_angle3
                          + self.Kd_angle3 * d_angle3)
        ...
        # Phase 4: Autonomous Forward and Backward (single hardware rfb command execution)
        if self.phase == self.PHASE_FWD_BWD:
            target_h = float(self.target_heading) if self.target_heading is not None else bearing_heading
            ang_err = wrap_to_180(current_angle - target_h)
            if self._rfb_start_time is None:
                self._rfb_start_time = current_time

            total_rfb_duration = 2.0 * self.fwd_bwd_time_s
            elapsed_rfb = current_time - self._rfb_start_time

            rfb_trigger = None
            if not self._rfb_triggered:
                self._rfb_triggered = True
                duration_ms = int(self.fwd_bwd_time_s * 1000)
                rfb_trigger = (self.fwd_bwd_speed, duration_ms)

            if elapsed_rfb >= total_rfb_duration:
                self.phase = self.PHASE_COMPLETED
                debug = self._debug(
                    self.PHASE_COMPLETED, distance_error, target_h,
                    ang_err, 0.0, 0.0, True,
                )
                if rfb_trigger is not None:
                    debug['rfb_trigger'] = rfb_trigger
                return 0, 0, debug
            else:
                debug = self._debug(
                    self.PHASE_FWD_BWD, distance_error, target_h,
                    ang_err, 0.0, 0.0, False,
                )
                debug['elapsed_rfb'] = elapsed_rfb
                debug['remaining_rfb'] = max(0.0, total_rfb_duration - elapsed_rfb)
                if rfb_trigger is not None:
                    debug['rfb_trigger'] = rfb_trigger
                # Speed is 0, 0 because motion is handled autonomously by Leanbot firmware via rfb
                return 0, 0, debug

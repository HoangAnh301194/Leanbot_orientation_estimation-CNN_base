...
def main():
    ...
    parser.add_argument("--final-angle", type=float, default=None, help="Goc xoay cuoi cung sau khi den dich (neu None thi khong xoay)")
    ...
    def on_mouse_click(event, x, y, flags, param):
        nonlocal target_pos, is_pid_completed, is_auto_pid_enabled
        ...
        while True:
            ...
            if is_pid_completed:
                ...
                if target_pos is not None:
                    pid_status_txt = f"NAV: Target=({target_pos[0]}, {target_pos[1]}) [STOPPED / COMPLETED]"
                else:
                    pid_status_txt = f"PID: L=   0 R=   0 | Target: {args.target_angle:.1f}deg [STOPPED / COMPLETED]"
                ...
                if sm_fused is not None and detected:
                    speed_l, speed_r, dbg_pos = pos_pid.compute(
                        ...
                        final_heading_deg=args.final_angle
                    )
                    dist_err = dbg_pos["distance"]
                    ang_err = dbg_pos["angle_error"]
                    pos_at_setpoint = dbg_pos["is_completed"]
                    pos_state = "AT_SETPOINT" if pos_at_setpoint else dbg_pos["state"]
                    if pos_at_setpoint:
                    ...
                    pid_color_type = "ALIGNED" if pos_at_setpoint else ("AUTO" if is_auto_pid_enabled else "PAUSED")
                else:
                    speed_l, speed_r = 0, 0
                    pos_pid.reset()
                ...
                if pid_aligned:
                    pid_status_txt += " [AT SETPOINT / TRACKING]"
                    is_pid_completed = True

...
def plot_pid_log(
    ...
    # Look up kp_angle2 and kd_angle2 in experiment_index.csv if present
    exp_idx_path = Path(csv_path).parent / "experiment_index.csv"
    if exp_idx_path.exists():
        try:
            exp_df = pd.read_csv(exp_idx_path)
            matching = exp_df[exp_df["log_file"] == Path(csv_path).name]
            if len(matching) > 0:
                if "kp_angle2" in matching.columns and not pd.isna(matching["kp_angle2"].iloc[0]):
                    kp_angle2 = float(matching["kp_angle2"].iloc[0])
                if "kd_angle2" in matching.columns and not pd.isna(matching["kd_angle2"].iloc[0]):
                    kd_angle2 = float(matching["kd_angle2"].iloc[0])
        except Exception:
            pass
    ...
    if "x_center" in df.columns and "y_center" in df.columns:
        ...
        # Trích xuất góc target_heading cho mũi tên mục tiêu
        trg_h = None
        ...
        if trg_h is None and "target_angle" in df.columns and "pid_mode" in df.columns:
            p4_sub = df[df["pid_mode"].astype(str).str.contains("PHASE_4")]
            if len(p4_sub) > 0:
                non_zero = p4_sub["target_angle"].dropna()
                if len(non_zero) > 0:
                    trg_h = float(non_zero.iloc[0])
        if trg_h is None:
            # Fallback đọc từ target_config.json
            candidate_dirs = [
                os.path.dirname(os.path.abspath(csv_path)),
                os.path.abspath(os.path.join(os.path.dirname(csv_path), "..")),
                os.getcwd()
            ]
            for cdir in candidate_dirs:
                cfile = os.path.join(cdir, "target_config.json")
                if os.path.exists(cfile):
                    try:
                        with open(cfile, "r") as f:
                            cdata = json.load(f)
                            if "target_heading" in cdata:
                                trg_h = float(cdata["target_heading"])
                                break
                    except Exception:
                        pass

        # Lọc bỏ frame lost tracking: đặt thành NaN để ngắt đường vẽ (discontinuous)
        traj_df = df.copy()
        if "tracking_lost" in traj_df.columns:
            is_lost = (traj_df["tracking_lost"] == 1) | (traj_df["tracking_lost"] == "1") | (traj_df["tracking_lost"] == True)
            traj_df.loc[is_lost, ["x_center", "y_center"]] = np.nan
        ...
        if len(valid_coords) > 0:
            ...
            plt.scatter(start_x, start_y, color="green", s=120, zorder=5, label=f"Start ({start_x:.0f}, {start_y:.0f})")
            ...
            end_x = valid_coords["x_center"].iloc[-1]
            end_y = valid_coords["y_center"].iloc[-1]
            plt.scatter(end_x, end_y, color="blue", s=120, zorder=5, label=f"End ({end_x:.0f}, {end_y:.0f})")

            # Vẽ điểm Target Goal và đoạn mũi tên ngắn màu vàng chỉ hướng target heading
            if "target_x" in df.columns and (df["target_x"] > 0).any():
                targets = df[df["target_x"] > 0][["target_x", "target_y"]].drop_duplicates()
                has_arrow_legend = False
                for _, trg in targets.iterrows():
                    tx = float(trg["target_x"])
                    ty = float(trg["target_y"])
                    plt.scatter(tx, ty, color="red", marker="*", s=250, zorder=6, label=f"Target Goal ({tx:.0f}, {ty:.0f})")

                    if trg_h is not None:
                        # Đoạn mũi tên màu vàng chỉ hướng Target Heading (82.5 px - dài gấp 1.5 lần)
                        rad_th = np.radians(trg_h)
                        arrow_len = 82.5
                        dx = arrow_len * np.cos(rad_th)
                        dy = -arrow_len * np.sin(rad_th)  # Trục Y đảo ngược trong ảnh
                        plt.annotate(
                            "",
                            xy=(tx + dx, ty + dy),
                            xytext=(tx, ty),
                        ...
                        if not has_arrow_legend:
                            plt.plot([], [], color="#FFB800", linewidth=3.2, label=f"Target Heading ({trg_h:.1f}°)")
                            has_arrow_legend = True
    ...
    if "pid_mode" in df.columns and "x_center" in df.columns and "y_center" in df.columns:
        p4_fwd_bwd = df[df["pid_mode"] == "PHASE_4_FWD_BWD"].dropna(subset=["x_center", "y_center"])
        if "tracking_lost" in p4_fwd_bwd.columns:
            p4_fwd_bwd = p4_fwd_bwd[(p4_fwd_bwd["tracking_lost"] == 0) | (p4_fwd_bwd["tracking_lost"] == "0")]

        if len(p4_fwd_bwd) >= 6:
            # Tách tiến và lùi dựa vào đỉnh distance_error xa target nhất
            if "distance_error" in p4_fwd_bwd.columns and (p4_fwd_bwd["distance_error"] > 0).any():
                turnaround_idx = p4_fwd_bwd["distance_error"].idxmax()
                fwd_df = p4_fwd_bwd.loc[:turnaround_idx]
                bwd_df = p4_fwd_bwd.loc[turnaround_idx:]
            else:
                half = len(p4_fwd_bwd) // 2
                fwd_df = p4_fwd_bwd.iloc[:half]
                bwd_df = p4_fwd_bwd.iloc[half:]
        else:
        ...
        if len(fwd_df) >= 3 or len(bwd_df) >= 3:
            ...
            # Target heading: lấy từ PHASE_3_FINAL_ALIGNING hoặc target_config/experiment_index
            trg_h = None
            if "target_angle" in df.columns and "pid_mode" in df.columns:
                p3_sub = df[df["pid_mode"] == "PHASE_3_FINAL_ALIGNING"]
                if len(p3_sub) > 0:
            ...
            if trg_h is None and "target_angle" in fit_df.columns:
                ...
                if len(non_zero) > 0:
                    trg_h = float(non_zero.iloc[0])
            if trg_h is None:
                # Check experiment_index.csv
                exp_idx_path = Path(csv_path).parent / "experiment_index.csv"
                if exp_idx_path.exists():
                    try:
                        exp_df = pd.read_csv(exp_idx_path)
                        matching = exp_df[exp_df["log_file"] == Path(csv_path).name]
                        if len(matching) > 0 and "target_heading_deg" in matching.columns:
                            trg_h = float(matching["target_heading_deg"].iloc[0])
                    except Exception:
                        pass
            if trg_h is None:
                trg_h = 0.0
...
def main():
    ...
    if args.log:
        ...
        csv_files = glob.glob(str(local_benchmark_dir / "log_*.csv"))
        if not csv_files:
            csv_files = glob.glob(str(parent_benchmark_dir / "log_*.csv"))
        if not csv_files:

...
def run_realtime_inference(model, frame, args, csv_writer, frame_id, timestamp):
    ...
    total_anchors = len(raw_class_scores)
    ...
    orig_boxes_xyxy = check_confidence.restore_boxes_from_training_style(top_boxes_xyxy, params)
...
def main():
    ...
    parser.add_argument("--topk", type=int, default=200, help="Top-K anchors (default 200)")
    ...
    print(f"File log CSV duoc luu tai: {csv_path}")
    ...
    try:
        ...
        print(f"Da dong camera, file log duoc ghi tai {csv_path}")

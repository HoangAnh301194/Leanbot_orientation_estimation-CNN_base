        ...
        while True:
            ...
            if not args.no_show:
                ...
                if recording and tracking_lost:
                    capture_name = f"lost_frame_{frame_id}_{safe_timestamp_for_filename(timestamp)}_{inference_mode}.png"
                    capture_path = os.path.join(lost_capture_dir, capture_name)
                    cv2.imwrite(capture_path, display_frame)
                    print(f"[INFO] Saved lost tracking capture: {capture_path}")

...
def sliding_window_poly2(x_arr: np.ndarray, y_arr: np.ndarray, window_size: int = 18):
    """Tính toán Polynomial Bậc 2 trên Causal Sliding Window (SMOOTH_LENGTH=18).
    Chỉ sử dụng mẫu hiện tại và quá khứ (t_norm in [-1.0, 0.0], t=0 là mẫu hiện tại).
    Không sử dụng mẫu tương lai (No future samples).
    Trả về vị trí mượt (x_smooth, y_smooth) và đạo hàm (dx_dt, dy_dt) tại mẫu hiện tại t=0.
    """
    n = len(x_arr)
    x_smooth = np.zeros(n)
    y_smooth = np.zeros(n)
    ...
    for i in range(n):
        ...
        # Tại t_norm = 0.0 (tương ứng với mẫu hiện tại i)

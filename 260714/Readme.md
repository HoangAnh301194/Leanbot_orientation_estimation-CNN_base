...
#### 1.2 Chạy inference với YOLO11n OpenVINO FP16 ở chế độ ROI tracking
...
```bash
python .\tools\roi_tracking_baseline_infer.py --source 1 --mode roi --log benchmark\yolo11n_fp16_roi_tracking.csv --show --full-model models\YOLO11n_versions\quantized_fp16\Soft_Angular_BCE_yolo11n_openvino_model --tracking-model models\YOLO11n_versions\Soft_Angular_BCE_yolo11n_static_160_openvino_model
```
- Lệnh chạy với YOLOv8n OpenVINO FP16 ở chế độ ROI tracking.

```bash
python .\tools\roi_tracking_baseline_infer.py --source 1 --mode roi --log benchmark\yolov8n_fp16_roi_tracking.csv --show --full-model models\YOLOv8n_versions\quantized_fp16\best_24Class_Soft_Angular_BCE_openvino_model --tracking-model models\YOLOv8n_versions\best_24Class_Soft_Angular_BCE_static_160_openvino_model
``` 

- Lệnh chạy phân tích và tự động vẽ biểu đồ:

```bash
python .\tools\comprehensive_plot.py
```
...
#### 1.3. Thời gian training YOLOv8n và YOLO11n Soft Angular BCE
...
- Trong quá trình chạy thử nghiệm ROI Tracking, mô hình **YOLOv8n** gặp tình trạng bị mất dấu (Tracking Lost) tại một số khung hình:

![Lost Frame 2307 (Full)](./benchmark/lost_tracking_captures/lost_frame_2307_08-44-42-396_ROI_frame.png)

*Hình 1: Toàn cảnh khung hình 2307 lúc bị mất dấu mục tiêu*

![Lost Frame 2307 (ROI Crop)](./benchmark/lost_tracking_captures/lost_frame_2307_08-44-42-396_ROI_roi.png)

*Hình 2: Ảnh crop ROI 160x160 đầu vào của model Tracking tại khung hình 2307*

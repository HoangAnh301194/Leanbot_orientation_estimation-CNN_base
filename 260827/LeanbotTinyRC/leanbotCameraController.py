...
class BLEMotorWorker:
    ...
    async def _async_loop(self):
        ...
        try:
            ...
            self.connected = True
            print(f"[INFO] Ket noi BLE thanh cong voi Leanbot {self.leanbot_id}! Navigation [READY]")
...
def main():
    ...
    if args.no_show:
        ...
        start_recording()
        print("[INFO] Phim dieu khien: S = bat dau, P = tam dung, C = huy luot, R = ghi log, Q = thoat.")
    else:
        print("[INFO] Phim dieu khien: S = bat dau, P = tam dung, C = huy luot, R = ghi log, Q = thoat.")
    ...
    print(f"[INFO] P-only Navigation | BLE Leanbot: {args.ble if args.ble > 0 else 'OFF'}")
    print("[INFO] Target se co dinh tai tam frame. Nhan S de Leanbot bat dau di den target.")
    ...
    try:
        while True:
            ...
            if is_pid_completed:
                ...
                nav_sub_txt = "P-only: L=   0 R=   0 | Giu nguyen giai doan hien tai"
                pid_color_type = "LOST"
            ...
            if not args.no_show:
                ...
                try:
                    ...
                    if key in (ord('p'), ord('P')):
                        is_auto_pid_enabled = False
                        print("[INFO] PAUSED. Nhan S de bat dau lai tu giai doan 1.")
                        if ble_worker is not None:
                            ble_worker.send_speed(0, 0)
                    if key in (ord('s'), ord('S')):
                        ...
                        print(f"[INFO] START: target co dinh {target_pos}. Bat dau giai doan 1.")
                        if ble_worker is not None:
                            ble_worker.send_speed(0, 0)
                    ...
                    if key in (ord('c'), ord('C')):
                        ...
                        if ble_worker is not None:
                            ble_worker.send_speed(0, 0)
                        print("\n[KEY 'c'] DA HUY LUOT CHAY -> Dung xe; target tam frame duoc giu nguyen.\n")

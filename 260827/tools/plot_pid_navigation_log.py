...
Ho tro ve:
  ...
  5. Do thi Hieu nang (FPS, Inference Latency, End-to-End Latency).
"""
...
def plot_pid_log(csv_path: str, save_dir: str = ""):
    ...
    # Subplot 1: Goc (Model, Trajectory, Fused, Target)
    ax1 = axes[0]
    if "raw_angle" in df.columns:
        ax1.plot(frames, df["raw_angle"], label="Raw Model Angle", color="lightgray", alpha=0.6, linewidth=1, linestyle=":")
    if "model_angle_smooth" in df.columns:
        ax1.plot(frames, df["model_angle_smooth"], label="Model Smooth (W=18)", color="#1f77b4", linewidth=1.5)
    if "trajectory_angle_smooth" in df.columns:
        ax1.plot(frames, df["trajectory_angle_smooth"], label="Trajectory Smooth", color="#ff7f0e", linewidth=1.2, linestyle="--")
    if "fused_angle" in df.columns:
        ax1.plot(frames, df["fused_angle"], label="Fused Angle (Control)", color="#2ca02c", linewidth=2.0)
    if "target_angle" in df.columns and (df["target_angle"] != 0).any():
        ax1.plot(frames, df["target_angle"], label="Target Angle", color="red", linewidth=1.8, linestyle="-.")

    ax1.set_ylabel("Góc (Độ)", fontsize=11)
    ax1.set_title("1. Đáp ứng Góc của Leanbot (Model vs Trajectory vs Fused vs Target)", fontsize=12, fontweight="bold")
    ...
    if "x_center" in df.columns and "y_center" in df.columns:
        ...
        if len(valid_coords) > 0:
            ...
            plt.gca().invert_yaxis()  # Anh camera goc toa do (0,0) o goc tren ben trai
...
def main():
    ...
    parser.add_argument("--log", default="", help="Duong dan toi file CSV log (neu khong truyen se tu chon file moi nhat trong benchmark/)")
    parser.add_argument("--out", default="", help="Thu muc luu anh do thi (default: benchmark/plots/)")
    ...
    if args.log:
        ...
        # Tim file log CSV moi nhat trong benchmark/
        csv_files = glob.glob(str(benchmark_dir / "*.csv"))
        if not csv_files:
            print(f"[WARN] Khong tim thay file CSV nao trong {benchmark_dir}")
            return

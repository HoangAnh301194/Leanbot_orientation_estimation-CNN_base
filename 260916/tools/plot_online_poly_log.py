...
def plot_online_poly_log(csv_file: str, n_sample_segments: int = 3, poly_degree: int = 2):
    ...
    has_smooth2 = 'smooth_x2' in df.columns and 'smooth_y2' in df.columns
    ...
    # ─────────────────────────────────────────────────────────────────────────
    # 1. Đồ thị Quỹ đạo 2D (Raw vs Smooth vs Smooth2)
    #    - Raw: màu đỏ (#d62728), marker 'o', đường nối liền
    #    - Smooth 1: màu xanh dương (#1f77b4), marker 'x', đường nối liền
    #    - Smooth 2: màu xanh lá (#2ca02c), marker '+', đường nối liền
    # ─────────────────────────────────────────────────────────────────────────
    fig_traj, ax_traj = plt.subplots(figsize=(10, 8), dpi=150)
    
    if has_raw:
        # Đường Raw: màu đỏ, nét nối liền, marker tròn O
        ax_traj.plot(df['x_center'], df['y_center'], color='#d62728', linewidth=1.2, linestyle='-', alpha=0.7, label='Raw Trajectory (O)', zorder=1)
        ax_traj.scatter(df['x_center'], df['y_center'], color='#8b0000', s=12, marker='o', alpha=0.8, zorder=2)

    if has_smooth:
        # Đường Smooth 1st Pass: màu xanh dương, nét nối liền, marker chữ X
        ax_traj.plot(df['smooth_x'], df['smooth_y'], color='#1f77b4', linewidth=1.6, linestyle='-', alpha=0.85, label='Smooth 1st Pass (X)', zorder=3)
        ax_traj.scatter(df['smooth_x'], df['smooth_y'], color='#003366', s=16, marker='x', alpha=0.9, zorder=4)

    if has_smooth2:
        # Đường Smooth 2nd Pass: màu xanh lá, nét nối liền, marker dấu +
        ax_traj.plot(df['smooth_x2'], df['smooth_y2'], color='#2ca02c', linewidth=2.0, linestyle='-', alpha=0.95, label='Smooth 2nd Pass (+)', zorder=5)
        ax_traj.scatter(df['smooth_x2'], df['smooth_y2'], color='#006600', s=24, marker='+', alpha=1.0, zorder=6)

    ax_traj.invert_yaxis()
    ax_traj.set_title(f'2D Trajectory - Poly Smooth (deg={poly_degree})\n{csv_path.name}', fontsize=14, fontweight='bold')
    ...
    # ─────────────────────────────────────────────────────────────────────────
    # 2. Phân tích & Trích xuất các đoạn 30 điểm (Chuẩn hóa t in [-1.0, 0.0])
    # ─────────────────────────────────────────────────────────────────────────
    ...
    if n_points >= seg_len:
        print(f"\n--- ĐA THỨC BẬC {poly_degree} f_x(t) & f_y(t) CHO CÁC ĐOẠN 30 ĐIỂM (t in [-1, 0]) ---")
        indices = np.linspace(0, n_points - seg_len, num=min(n_sample_segments, max(1, n_points // seg_len)), dtype=int)
        
        # Pre-compute mảng thời gian t chuẩn hóa 1 lần duy nhất trong [-1.0, 0.0]
        t_norm = np.linspace(-1.0, 0.0, seg_len)
        ...
        for idx_i, start_idx in enumerate(indices):
            ...
            # Vẽ đồ thị thành phần X(t) và Y(t) theo t chuẩn hóa [-1, 0]
            fig_comp, (ax_x, ax_y) = plt.subplots(2, 1, figsize=(10, 6), sharex=True, dpi=150)
            t_dense = np.linspace(-1.0, 0.0, 100)
            ...
            ax_y.set_xlabel('Normalized Time t in [-1, 0]')
    ...
    if has_angle:
        ...
        ax_ang.plot(df['frame_id'], raw_unwrapped, color='#8b0000', linewidth=2.5, linestyle='-', alpha=1.0, label='Raw Angle (Model)', zorder=10)
    ...
    if smooth_angle is not None:
        ...
        ax_ang.plot(df['frame_id'], smooth_unwrapped, color='#ff7f00', linewidth=1.2, linestyle='-', alpha=0.6, label='Smooth Angle')
    ...
    if 'raw_angle_smooth' in df.columns:
        ...
        ax_ang.plot(df['frame_id'], raw_angle_smooth, color='#2ca02c', linewidth=1.8, linestyle='-', label='Raw Angle Smooth (Poly)')

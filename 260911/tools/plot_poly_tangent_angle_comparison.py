...
def calculate_angle_series(
    ...
    window_size_smooth2: int | None = None,
):
    if window_size_smooth2 is None:
    ...
    tangent_smooth2 = fit_causal_endpoint_1d(
        tangent_aligned,
        window_size=window_size_smooth2,
        poly_degree=poly_degree,
    )
    ...
    return {
        ...
        "window_size_smooth2": window_size_smooth2,
    }
...
def save_trajectory_comparison(
    ...
    deg_suffix = f"_deg{poly_degree}" if poly_degree != DEFAULT_POLY_DEGREE else ""
    output_path = output_dir / (
        f"{csv_path.stem}_poly_least_squares_2d_trajectory{deg_suffix}_w{window_size}.png"
    )
    figure.savefig(
        output_path,
        dpi=dpi,
    ...
    if window_size == DEFAULT_WINDOW_SIZE and poly_degree == DEFAULT_POLY_DEGREE:
...
def save_4line_angle_plot(
    ...
    axis.plot(
        ...
        linestyle="-",
        alpha=0.9,
        label=f"Polynomial Endpoint Tangent Angle (Smooth 1, Degree {poly_degree}, W={window_size})",
        zorder=7,
    )
    axis.plot(
        plot_frames,
        ...
        linestyle="-",
        alpha=0.95,
        label=f"Polynomial Endpoint Tangent Angle Smooth2 (Smooth 2, Degree {poly_degree}, W={window_size_smooth2})",
        zorder=8,
    )

    deg_label = "Bậc 1 (Linear)" if poly_degree == 1 else f"Degree={poly_degree}"
    axis.set_title(
        f"Heading Angle Comparison: Raw Model vs Consecutive Smoothed Points "
        f"vs Endpoint Tangent vs Smooth2\n"
        f"File: {csv_path.name} | Frames {frame_start}-{frame_end} | "
        f"Causal W1={window_size}, W2={window_size_smooth2}, Polynomial {deg_label}",
        fontsize=12,
        fontweight="bold",
    )
    axis.set_xlabel("Frame ID", fontweight="bold")
    ...
    output_dir.mkdir(parents=True, exist_ok=True)
    deg_suffix = f"_deg{poly_degree}" if poly_degree != DEFAULT_POLY_DEGREE else ""
    out_path = output_dir / f"{csv_path.stem}_poly_tangent_angle_comparison{deg_suffix}_w{window_size}.png"
    figure.savefig(out_path, dpi=dpi, bbox_inches="tight", pad_inches=0.15)
    plt.close(figure)
...
def plot_angle_comparison(
    ...
    w2_val = angle_series["window_size_smooth2"]
    ...
    # Chỉ lưu đồ thị 3 đường khi chạy mặc định bậc 2
    if poly_degree == DEFAULT_POLY_DEGREE:
    ...
    angle_output_path = save_4line_angle_plot(
        ...
        smooth_point_plot=smooth_point_plot,
        window_size=window_size,
        window_size_smooth2=w2_val,
        poly_degree=poly_degree,
        dpi=dpi,
    )

    print(
        f"[INFO] {csv_path.name} (W1={window_size}, W2={w2_val}): {len(segment)} mẫu, "
    ...
    print_angle_summary(f"Polynomial Tangent Smooth2 (Smooth 2, W={w2_val})", tangent_smooth2_plot)
...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        help=f"Kích thước cửa sổ causal cho Smooth 1, mặc định {DEFAULT_WINDOW_SIZE}",
    )
    parser.add_argument(
        "--smooth2-window-size",
        type=int,
        default=None,
        help="Kích thước cửa sổ causal cho Smooth 2, mặc định là window-size * 2",
    )
    parser.add_argument(
        "--poly-degree",
        type=int,
...
def main() -> int:
    ...
    w2_info = (
        f", smooth2-window-size={arguments.smooth2_window_size}"
        if arguments.smooth2_window_size is not None
        else f", smooth2-window-size={arguments.window_size * 2} (auto W*2)"
    )
    print(f"[INFO] Tìm thấy {len(csv_files)} file CSV (window-size={arguments.window_size}{w2_info})")
    failure_count = 0
    for csv_path in csv_files:
        try:
            plot_angle_comparison(
                ...
                dpi=arguments.dpi,
                window_size_smooth2=arguments.smooth2_window_size,
            )

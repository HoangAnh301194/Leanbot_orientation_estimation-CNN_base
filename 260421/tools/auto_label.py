...
from abstract_hsv import compute_hsv_diff, compute_gray_hue_diff
...
def detect_leanbot(frame, bg_bgr, aligner, board_mask, diff_mode='1', threshold=50, blur_ksize=5,
    ...
    # 3. Tính sự khác biệt (Trừ nền) dựa trên mode
    if diff_mode == '1':
        # Mode 1: Grayscale (Baseline)
        # Tự tính preprocess ở đây để dùng blur_ksize tùy chỉnh
...
def main():
    ...
    print(f"\n[DEBUG] Thuật toán đang sử dụng: {mode_names.get(args.diff_mode, 'Unknown')}")
    if args.diff_mode == '1':
    ...
    for session_dir, frames in sessions_to_process:
        ...
        for i, frame in enumerate(frames):
            aligned_img, bboxes, diff_mask = detect_leanbot(
                frame, bg_masked, s_aligner, board_mask, 

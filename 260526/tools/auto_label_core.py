...
def resolve_capture_session(session_arg: str | list[str] | None):
    ensure_capture_root()
    all_sessions = sorted([path for path in RAW_IMAGE_ROOT.iterdir() if path.is_dir()])
    
    if session_arg is None:
        return all_sessions
        
    if isinstance(session_arg, str):
        session_arg = [session_arg]
        
    resolved_sessions = []
    for arg in session_arg:
        if arg == "latest":
            if all_sessions:
                resolved_sessions.append(all_sessions[-1])
            continue
            
        requested_path = Path(arg)
        candidate = requested_path if requested_path.exists() else RAW_IMAGE_ROOT / arg
        if candidate.is_dir():
            resolved_sessions.append(candidate)
        else:
            print(f"[WARN] Session not found: {arg}")
            
    if not resolved_sessions:
        raise FileNotFoundError(f"No valid sessions found from: {session_arg}")
...
def add_processing_arguments(parser: argparse.ArgumentParser):
    ...
    parser.add_argument("--min_area", type=int, default=6000, help="Minimum contour area")

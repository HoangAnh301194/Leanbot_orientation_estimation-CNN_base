...
from PID_controller import create_default_pid, create_position_pid, PIDController, PositionPIDController
...
def select_best_vector_detection(compiled_model, image, names,
    ...
    Returns: (box_xyxy, best_conf, angle, vector_magnitude, angle2, mag2)
    """
...
class BLEMotorWorker:
    """Chay BLE Leanbot tren mot Background Thread rieng biet de tranh xung dot COM STA / Windows GUI voi OpenCV."""
    ...
    async def _async_loop(self):
        ...
        try:
            print(f"[INFO] Dang ket noi toi Leanbot {self.leanbot_id} qua BLE...")
            await self.leanbot.find()
            await self.leanbot.connect()
            self.connected = True
            print(f"[INFO] Ket noi BLE thanh cong voi Leanbot {self.leanbot_id}! Auto-PID [ON]")
            ...
            while self.running:
                ...
                if cmd is not None:
                    try:
                        ...
                        print(f"[WARN] Loi gui BLE: {e}")
                
                await asyncio.sleep(0.02)
        except Exception as e:
            print(f"[ERROR] Khong the ket noi BLE voi Leanbot {self.leanbot_id}: {e}")
        finally:
...
def main():
    ...
    parser.add_argument("--width", type=int, default=1280, help="Chieu rong camera mong muon (default 1280)")
    parser.add_argument("--height", type=int, default=720, help="Chieu cao camera mong muon (default 720)")
    ...
    parser.add_argument("--fps", type=float, default=30.0, help="Gioi han FPS toi da (default 30.0, vd: 25 hoac 30)")
    # --- Tham so pipeline lam muot goc ---
    parser.add_argument("--smooth-window", type=int, default=18, help="Kich thuoc cua so truot lam muot (default 18)")
    parser.add_argument("--smooth-index", type=int, default=-4, help="Delay tangent index (default -4)")
    parser.add_argument("--smooth-K", type=float, default=3.0, help="He so K cho fused angle (default 3.0)")
    parser.add_argument("--ble", type=int, default=0, help="ID cua Leanbot de ket noi BLE (vd: --ble 123456, default 0: tat)")
    parser.add_argument("--target-angle", type=float, default=90.0, help="Goc muc tieu mong muon (default 90.0 deg)")
    parser.add_argument("--hold-ms", type=float, default=100.0, help="Thoi gian giu goc on dinh truoc khi dung han (default 100.0 ms)")
    ...
    def start_recording():
        ...
        log_handle.flush()
        recording = True
        print(f"\n[INFO] >>> REC ON. Bat dau ghi file log moi tai: {current_log_file} <<<\n")
    ...
    def stop_recording():
        ...
        writer = None
        recording = False
        print(f"\n[INFO] >>> REC OFF. Da luu xong file log tai: {current_log_file} <<<\n")

    print(f"[INFO] Bat dau Inference ({args.mode.upper()} mode).")
    if args.no_show:
        print(f"[INFO] No-show mode: tu ghi log vao {benchmark_base}")
        start_recording()
    elif args.video:
        print(f"[INFO] Video mode: tu dong ghi log vao {benchmark_base}")
        start_recording()
    ...
    # --- Khoi tao bo dieu khien PID & BLE Worker ---
    pid = create_default_pid()
    pos_pid = create_position_pid()
    ble_worker = BLEMotorWorker(args.ble) if args.ble > 0 else None
    is_auto_pid_enabled = (args.ble > 0)
    aligned_start_time = None
    is_pid_completed = False
    target_pos = None  # (target_x, target_y) khi click chuot

    def on_mouse_click(event, x, y, flags, param):
        nonlocal target_pos, is_pid_completed, is_auto_pid_enabled, aligned_start_time
        if event == cv2.EVENT_LBUTTONDOWN:
            # Quy doi toa do click tren cua so thu nho ve toa do anh goc
            scale_x = img_w / max(1, disp_w)
            scale_y = img_h / max(1, disp_h)
            orig_tx = int(x * scale_x)
            orig_ty = int(y * scale_y)
            target_pos = (orig_tx, orig_ty)
            is_pid_completed = False
            is_auto_pid_enabled = True
            aligned_start_time = None
            pos_pid.reset()
            print(f"\n[CLICK TO NAVIGATE] Chon muc tieu moi: Target=({orig_tx}, {orig_ty}) -> Auto Navigate [ON]!\n")
            ...
            print("\n[CLICK TO NAVIGATE] DA HUY MUC TIEU -> Dung xe (0, 0) va xoa Target Marker!\n")

    # --- Khoi tao engine lam muot goc ---
    smoothing_engine = AngleSmoothingEngine(
        window_size=args.smooth_window,
        eval_index=args.smooth_index,
        K=args.smooth_K,
    )
    print(f"[INFO] Angle smoothing: W={args.smooth_window}, index={args.smooth_index}, K={args.smooth_K}")
    print(f"[INFO] PID Target Angle: {args.target_angle:.1f} deg | BLE Leanbot: {args.ble if args.ble > 0 else 'OFF'}")

    # --- Realtime angle graph ---
    GRAPH_W, GRAPH_H = 600, 300
    GRAPH_MAX_POINTS = 200  # so frame hien thi tren do thi
    ...
    # Mau sac (BGR)
    COLOR_MODEL = (255, 120, 0)    # Xanh duong
    COLOR_TRAJ  = (0, 140, 255)    # Cam
    COLOR_FUSED = (0, 220, 0)      # Xanh la

    # Pre-allocated canvas de tiet kiem CPU
    graph_canvas = np.zeros((GRAPH_H, GRAPH_W, 3), dtype=np.uint8)

    def draw_angle_graph():
        """Ve do thi 3 duong goc realtime tren canvas OpenCV (tai su dung bo nho)."""
        graph_canvas.fill(30)  # nen xam dam
    ...
    try:
        while True:
            ...
            if detected:
                ...
                if prev_bbox_xyxy is not None:
                    ...
                    iou_prev = 1.0  # Frame dau tien la 1.0 (100%)
            ...
            # Format gia tri smooth (None -> "NaN")
            def _fmt(v, fmt=".4f"):
                return f"{v:{fmt}}" if v is not None else "NaN"

            # --- Tinh toan PID dieu khien (Di chuyen toa do hoac Xoay goc) ---
            if is_pid_completed:
                speed_l, speed_r = 0, 0
                if target_pos is not None:
                    pid_status_txt = f"NAV: Target=({target_pos[0]}, {target_pos[1]}) [COMPLETED / REACHED]"
                else:
                    pid_status_txt = f"PID: L=   0 R=   0 | Target: {args.target_angle:.1f}deg [COMPLETED / STOPPED]"
                pid_color_type = "COMPLETED"
                nav_sub_txt = ""
            elif target_pos is not None:
                # --- Che do 1: Di chuyen toi toa do Pixel target (Position PID) ---
                if sm_fused is not None and detected:
                    speed_l, speed_r, dbg_pos = pos_pid.compute(
                        current_x=cx, current_y=cy, current_angle=sm_fused,
                        target_x=target_pos[0], target_y=target_pos[1]
                    )
                    dist_err = dbg_pos["distance"]
                    ang_err = dbg_pos["angle_error"]
                    pos_completed = dbg_pos["is_completed"]
                    pos_state = dbg_pos["state"]

                    if pos_completed and is_auto_pid_enabled:
                        if aligned_start_time is None:
                            aligned_start_time = time.perf_counter()
                        elapsed_aligned_ms = (time.perf_counter() - aligned_start_time) * 1000
                        if elapsed_aligned_ms >= args.hold_ms:
                            is_auto_pid_enabled = False
                            is_pid_completed = True
                            speed_l, speed_r = 0, 0
                            if ble_worker is not None:
                                ble_worker.send_speed(0, 0)
                            print(f"\n[INFO] >>> DA DEN DICH ({target_pos[0]}, {target_pos[1]}) ON DINH ({elapsed_aligned_ms:.0f}ms) -> DUNG [REACHED]! <<<\n")
                    else:
                        if not pos_completed:
                            aligned_start_time = None

                    pid_status_txt = f"Target: ({target_pos[0]}, {target_pos[1]}) | DistErr: {dist_err:.1f}px | State: {pos_state}"
                    nav_sub_txt = f"PID: L={speed_l:4d} R={speed_r:4d} | AngErr={ang_err:5.1f}deg {'[AUTO]' if is_auto_pid_enabled else '[PAUSED]'}"
                    pid_color_type = "COMPLETED" if pos_completed else ("AUTO" if is_auto_pid_enabled else "PAUSED")
                else:
                    speed_l, speed_r = 0, 0
                    pos_pid.reset()
                    pid_status_txt = f"Target: ({target_pos[0]}, {target_pos[1]}) [LOST TRACKING]"
                    nav_sub_txt = "PID: L=   0 R=   0"
                    pid_color_type = "LOST"
            elif sm_fused is not None:
                # --- Che do 2: Xoay tai cho ve goc muc tieu (Yaw Angle PID) ---
                nav_sub_txt = ""
                speed_l, speed_r, dbg_pid = pid.compute(
                    current_angle=sm_fused, target_angle=args.target_angle
                )
                pid_aligned = dbg_pid["is_aligned"]
                
                # Co che on dinh >= hold_ms (default 100ms): dung han va khoa PID
                if pid_aligned and is_auto_pid_enabled:
                    if aligned_start_time is None:
                        aligned_start_time = time.perf_counter()
                    elapsed_aligned_ms = (time.perf_counter() - aligned_start_time) * 1000
                    if elapsed_aligned_ms >= args.hold_ms:
                        ...
                        if ble_worker is not None:
                            ble_worker.send_speed(0, 0)
                        print(f"\n[INFO] >>> DA DAT GOC MUC TIEU ({args.target_angle:.1f} deg) ON DINH ({elapsed_aligned_ms:.0f}ms >= {args.hold_ms:.0f}ms) -> DUNG DIEU CHINH [COMPLETED]! <<<\n")
                else:
                    if not pid_aligned:
                        aligned_start_time = None
                ...
                if is_pid_completed:
                    ...
                    pid_status_txt = f"PID: L={speed_l:4d} R={speed_r:4d} | Err={dbg_pid['error']:5.1f}deg {'[AUTO]' if is_auto_pid_enabled else '[PAUSED]'}"
                    if pid_aligned:
                        hold_ms = int((time.perf_counter() - aligned_start_time) * 1000) if aligned_start_time else 0
                        pid_status_txt += f" (HOLD {hold_ms}ms)"
                    pid_color_type = "ALIGNED" if pid_aligned else ("AUTO" if is_auto_pid_enabled else "PAUSED")
            else:
                speed_l, speed_r = 0, 0
                pid.reset()
                aligned_start_time = None
                pid_status_txt = f"PID: L=   0 R=   0 | Target: {args.target_angle:.1f}deg [LOST TRACKING]"
                nav_sub_txt = ""
                pid_color_type = "LOST"

            # Gui lenh dieu khien toi Leanbot qua BLE Worker (thread-safe, non-blocking)
            if ble_worker is not None and is_auto_pid_enabled and not is_pid_completed:
                ble_worker.send_speed(speed_l, speed_r)

            if recording and writer is not None:
                # Xac dinh cac truong du lieu PID / BLE ghi CSV
                if target_pos is not None:
                    rec_pid_mode = "NAVIGATION"
                    rec_target_angle = 0.0
                    rec_angle_error = 0.0
                    rec_target_x = float(target_pos[0])
                    rec_target_y = float(target_pos[1])
                    rec_dist_error = float(dist_err) if (sm_fused is not None and detected) else 0.0
                else:
                    rec_pid_mode = "YAW_ANGLE"
                    rec_target_angle = float(args.target_angle)
                    rec_angle_error = float(dbg_pid["error"]) if (sm_fused is not None) else 0.0
                    rec_target_x = 0.0
                    rec_target_y = 0.0
                    rec_dist_error = 0.0
            ...
            if not args.no_show:
                ...
                # Ve diem muc tieu Target (Pixel) va duong noi mau vang tren detection_frame
                if target_pos is not None:
                ...
                # Giu nguyen ti le 16:9 toan canh (khong crop), giam kich thuoc 2 lan
                ...
                # Hien thi trang thai PID / Target Pixel / Pixel Error (Mau xanh den dam (180, 50, 0))
                if pid_color_type == "COMPLETED" or pid_color_type == "ALIGNED":
                    pid_color = (0, 255, 0)
                elif pid_color_type == "AUTO":
                    pid_color = (180, 50, 0)  # Xanh den dam (Navy Blue trong BGR)
                elif pid_color_type == "LOST":
                    pid_color = (0, 0, 255)
                else:
                ...
                try:
                    ...
                    if not windows_positioned:
                        ...
                        cv2.setMouseCallback("Leanbot Detection", on_mouse_click)
                    ...
                    if key == ord('p'):
                        if is_pid_completed:
                            ...
                            aligned_start_time = None
                            pid.reset()
                            print(f"[INFO] Auto-PID: RE-STARTED [ON]")
                        else:
                            is_auto_pid_enabled = not is_auto_pid_enabled
                            aligned_start_time = None
                            print(f"[INFO] Auto-PID: {'ENABLED [ON]' if is_auto_pid_enabled else 'PAUSED [OFF]'}")
                            if not is_auto_pid_enabled and ble_worker is not None:
                                ble_worker.send_speed(0, 0)
                    if key == ord('s'):
                        is_auto_pid_enabled = False
                        is_pid_completed = True
                        print("[INFO] Dung khan cap (STOP)!")
                        if ble_worker is not None:
                            ble_worker.send_speed(0, 0)
                    if key == ord('r'):
                        if recording:
                    ...
                    if key == ord('c'):
                        ...
                        if ble_worker is not None:
                            ble_worker.send_speed(0, 0)
                        print("\n[KEY 'c'] DA HUY MUC TIEU -> Dung xe (0, 0) va xoa Target Marker!\n")
                    if key == ord('k'):
        ...
        if ble_worker is not None:
            print("[INFO] Dang dung Leanbot va ngat ket noi BLE Worker...")
            ble_worker.stop()
        if log_handle is not None:
            log_handle.close()
    ...
    print(f"[INFO] Hoan tat. Thu muc log tai {benchmark_base}")
